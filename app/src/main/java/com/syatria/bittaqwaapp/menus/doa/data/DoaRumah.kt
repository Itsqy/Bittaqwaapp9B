    ...
    val listDoaRumahData: ArrayList<DoaModel>
        get() {
            ...
            for (position in doaTitle.indices) {
                val doa = DoaModel()
                doa.judulDoa = doaTitle[position]
                doa.arabDoa = doaDoa[position]
                doa.artiDoa = doaTranslate[position]
                doa.latinDoa = doaLatin[position]
                doa.riwayatDoa = doaProfile[position]
                list.add(doa)
            }
            return list
        }
